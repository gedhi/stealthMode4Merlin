...
stms=`basename ${0}`
# firmware check
IFMERLIN=`cat /etc/motd | grep "ASUSWRT-Merlin" | wc -l`
# LEDs status
...
if [ "$IFMERLIN" = "1" ]; then
    case "$1" in
        ...
        ;;
        *)
            echo "stealthMode Sunset v1.0 by Monter [released on 29.12.2014]"
    ...
    logger -p3 -s -t stealthMode Router does not support this feature. StealthMode works only in Asus with ASUSWRT-Merlin firmware.
fi
